...
public class Superstructure extends SubsystemBase {
    ...
    private final OI oi;
    private boolean shiftWarningArmed = true;
    private double rumbleStopTimeSec = 0.0;
    ...
    public void periodic() {
        ...
        double timeUntilHubStateChange = FieldConstants.getTimeUntilHubStateChange();
        ...
        Logger.recordOutput("Shooting/TimeUntilHubStateChange", timeUntilHubStateChange);
        Logger.recordOutput(
                "Shooting/DistanceToHub", round(vision.getHubDistanceMeasure().in(Meters) * 100.0) / 100.0);

        if (timeUntilHubStateChange == 0.0 || timeUntilHubStateChange > 5.0) {
            shiftWarningArmed = true;
        }
        if (shiftWarningArmed && timeUntilHubStateChange > 0.0 && timeUntilHubStateChange <= 5.0) {
            oi.setRumbleState(0.8, 0.8);
            rumbleStopTimeSec = Timer.getFPGATimestamp() + 0.4;
            shiftWarningArmed = false;
        } else if (Timer.getFPGATimestamp() >= rumbleStopTimeSec) {
            oi.setRumbleState(0.0, 0.0);
        }
